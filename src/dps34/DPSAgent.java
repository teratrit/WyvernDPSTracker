...
public class DPSAgent {
    ...
    public static void agentmain(String agentArgs, Instrumentation inst) {
        ...
        System.out.println("[DPS] Agent loaded (v34). Log: " + logPath);
    ...
    private static void processLine(String line, String styleName) {
        ...
        if (line.startsWith("Justice ") || line.startsWith("Dream Eater ")) {
            Matcher dmg = DAMAGE_PATTERN.matcher(line);
            if (dmg.find()) {
                writeEvent("OUT", dmg.group(1) + "|" + line);
            } else {
                writeEvent("DBG", "named_proc_no_damage|" + line);
            }
            return;
        }

        if (line.equals("You perform a ninja backstab.")) {
            writeEvent("BACKSTAB", "ninja");
            return;
        }
